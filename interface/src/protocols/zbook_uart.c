...
RPI_PICO_PIO_DEFINE_PROGRAM(uart_rx, 1, 8, 0x20a0, /*  0: wait   1 pin, 0                    */
						   /*  .wrap_target */
...
static struct {
	bool pio_uart_active;
	PIO pio_uart_pio;
	size_t pio_uart_tx_sm;
	size_t pio_uart_rx_sm;
} self;

static int pio_uart_tx_init(PIO pio, size_t sm, uint32_t tx_pin, float div, uint32_t *offset_out)
{
...
static void pio_uart_tx_deinit(PIO pio, size_t sm, uint32_t offset)
{
	pio_sm_set_enabled(pio, sm, false);
	pio_remove_program(pio, RPI_PICO_PIO_GET_PROGRAM(uart_tx), offset);
...
int zbook_uart_register_pin(uint32_t tx_pin, uint32_t rx_pin, uint32_t baudrate)
{
	...
	err = pio_uart_tx_init(pio, tx_sm, tx_pin, div, &tx_offset);
	if (err) {
		pio_sm_unclaim(pio, tx_sm);
	...
	if (err) {
		pio_uart_tx_deinit(pio, tx_sm, tx_offset);
		pio_sm_unclaim(pio, rx_sm);
		return err;
	}

	self.pio_uart_pio = pio;
	self.pio_uart_tx_sm = tx_sm;
	self.pio_uart_rx_sm = rx_sm;
	self.pio_uart_active = true;
...
{
	pio_sm_put_blocking(self.pio_uart_pio, self.pio_uart_tx_sm, (uint32_t)c);
}

static int pio_uart_read_byte(uint8_t *c)
{
	/* The RX FIFO is 4 bytes wide; the received byte lands in the MSB. */
	io_rw_8 *rx_fifo_msb = (io_rw_8 *)&self.pio_uart_pio->rxf[self.pio_uart_rx_sm] + 3;

	if (pio_sm_is_rx_fifo_empty(self.pio_uart_pio, self.pio_uart_rx_sm)) {
		return -EAGAIN;
	}
